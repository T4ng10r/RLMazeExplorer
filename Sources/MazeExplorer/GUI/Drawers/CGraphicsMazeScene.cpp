...
void CGraphicsMazeScene::setMaze(const CMaze & xMaze)
{
	//if (NULL == pMaze)	
	//{
	...
	reset();
    int iSizeX = xMaze.getXSize();
    int iSizeY = xMaze.getYSize();
    ...
    for(int iX = 0; iX<iSizeX; iX++)
	{
        ...
        {
            CLocation loc;
            if (false == xMaze.getLocation(iX,iY,loc))
            {
            ...
            ptrLocation->setLocationData(loc);
