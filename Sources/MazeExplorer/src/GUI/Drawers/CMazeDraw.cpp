...
void CMazeDraw::paintEvent(QPaintEvent *)
{
    ...
    for (index1 = 0; index1 < X; index1++)
        for (index2 = 0; index2 < Y; index2++)
        {
            ...
            boost::optional<location> location_ = pMaze->get_location(index1+1,index2+1);
            //DrawLocation(pMaze->getLocation(index1+1,index2+1), rect, &painter);
			DrawLocation(location_.get(), rect, &painter);
        }
...
void CMazeDraw::DrawLocation(const location& location, QRect rect, QPainter *painter)
{
...
void CMazeDraw::setMazeData(maze & mazeData)
{
    mazeData = *pMaze;
}
