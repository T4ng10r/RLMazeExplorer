...
namespace constants
{
	...
	const EMazeTypes maze_type(E_MT_RECURSIVE);
	const std::string maze_file_path("");
};

class ut_maze_recursive_test : public ::testing::Test
{
public:
	std::shared_ptr<maze_interface> uut;

	ut_maze_recursive_test()
	{
...
TEST_F(ut_maze_recursive_test, maze_sizes)
{
	EXPECT_EQ(uut->get_size_x(), constants::size_x);
	EXPECT_EQ(uut->get_size_y(), constants::size_y);
}

TEST_F(ut_maze_recursive_test, test_maze_edges)
{
...
TEST_F(ut_maze_recursive_test, test_maze_locations_integrity)
{
...
TEST_F(ut_maze_recursive_test, test_maze_locations_connections)
{
