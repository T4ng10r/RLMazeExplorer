...
void CEnviroment::startSingleExploring()
{
    ...
    emit robotFinished(m_stExplorationResult);
...
{
    m_stMaze.getLocation(m_iRobotPos-QPoint(1,1),m_stCurrentLocation);

    int size = !m_stCurrentLocation.up;
    scanResult->vFrontDir.resize(size);
    size = !m_stCurrentLocation.left;
    scanResult->vLeftDir.resize(size);
    size = !m_stCurrentLocation.right;
    scanResult->vRightDir.resize(size);
    size = !m_stCurrentLocation.down;
...
void CEnviroment::debugPrintMaze()
{
    ...
    QString mazeLine;
    QString mazeLine1;
    int maxX=m_stMaze.getXSize();
    int maxY=m_stMaze.getYSize();
    int indexX,indexY;
    ...
    for(indexY=0; indexY<maxY; indexY++)
    {
        ...
        {
			loc.Reset();
            m_stMaze.getLocation(QPoint(indexX, indexY),loc);
            if (loc.left)
                mazeLine+="|";
            else
            {
                if (!loc.right && loc.down)
                    mazeLine+="_";
                else
                    mazeLine+=" ";
            }
